...
use crate::agent::sidebar::model::{
    SidebarRow, SidebarRowIdentity, row_index_by_identity, row_index_by_window,
};

const CURRENT_VERSION: u32 = 2;

/// Tmux window option storing the preferred sidebar workspace row.
...
/// Return the persisted workspace row index when it still belongs to the sidebar context.
...
/// Encode a sidebar workspace row identity for storage in a tmux window option.
...
/// Decode a sidebar workspace row identity from a tmux window option value.
...
mod tests {
    ...
    #[test]
    fn manual_selection_prefers_workspace_identity_then_current_index() {
        let rows = vec![row("ses_a", "@1", "%1"), row("ses_b", "@2", "%2")];
        let identity = rows[1].identity.clone();

        assert_eq!(
            manual_selection_index(&rows, Some(&identity), Some(0)),
            Some(1)
        );
        assert_eq!(manual_selection_index(&rows, None, Some(1)), Some(1));
        assert_eq!(manual_selection_index(&rows, None, Some(9)), None);
    }
